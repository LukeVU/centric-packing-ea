...
for i in range(len(configs)):
    total_list.append((configs[i], average_list[i]))
...
for i in range(len(total_list)):
    plt.plot(range(len(total_list[i][1])), total_list[i][1], color=colors[i], linewidth=2, alpha=0.6, label=total_list[i][0])
...
tick_locations = np.arange(0, len(average_list[0]), len(average_list[0]) // 10)
...
plt.title(f"Average Fitness Of Each Config Over {num_runs} Runs")
